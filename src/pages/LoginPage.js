...
const LoginPage = () => {
    ...
    return (
        <div style={{
            ...
            {/* Form Column */}
            <div style={{ flex: '0 0 520px', minWidth: '450px', padding: '4rem 5rem', display: 'flex', flexDirection: 'column', zIndex: 10, background: 'rgba(0,0,0,0.8)' }}>
                ...
                <div style={{ maxWidth: '440px', marginBottom: 'auto' }}>
                    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}>
                        <h1 style={{ fontSize: '3rem', fontWeight: '900', lineHeight: '1', marginBottom: '1.5rem', letterSpacing: '-1px' }}>
                            Start Your<br />Evaluation<br /><span style={{ color: 'var(--primary)' }}>Today</span>
                        </h1>
                        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '15px', lineHeight: '1.6', marginBottom: '2.5rem' }}>
                            Please type your name and password below to enter the portal.
                        </p>
                    </motion.div>
                    ...
                    <div className="glass-card" style={{ padding: '2.5rem', borderRadius: '24px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2rem' }}>
                            ...
                            <span style={{ fontSize: '9px', fontWeight: '900', color: error ? 'var(--error)' : 'var(--primary)', letterSpacing: '1px' }}>
                                {loading ? 'PLEASE WAIT...' : error ? 'LOGIN FAILED' : 'WELCOME'}
                            </span>
                        </div>

                        <form onSubmit={handleEnter}>
                            <div style={{ marginBottom: '1.2rem' }}>
                                <label style={{ marginBottom: '0.8rem', display: 'block', fontSize: '10px', opacity: 0.4 }}>YOUR NAME</label>
                                <div style={{ position: 'relative' }}>
                                    <Cpu size={16} style={{ position: 'absolute', left: '1.2rem', top: '1.2rem', opacity: 0.2 }} />
                                    <input value={teamName} onChange={e => setTeamName(e.target.value)} placeholder="Full Name" style={{ width: '100%', paddingLeft: '3.2rem', height: '52px' }} required />
                                </div>
                            </div>

                            <div style={{ marginBottom: '1.8rem' }}>
                                <label style={{ marginBottom: '0.8rem', display: 'block', fontSize: '10px', opacity: 0.4 }}>PASSWORD (Code)</label>
                                <div style={{ position: 'relative' }}>
                            ...
                            {error && (
                                <div style={{ marginBottom: '1.5rem', color: 'var(--error)', background: 'rgba(248, 113, 113, 0.05)', padding: '1rem', borderRadius: '12px', border: '1px solid rgba(248, 113, 113, 0.1)', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '11px' }}>
                                    <ShieldAlert size={14} />
                                    <span style={{ fontWeight: '900' }}>{error}</span>
                                </div>
                            )}

                            <button disabled={loading} className="btn-primary" style={{ width: '100%', height: '54px', borderRadius: '50px', gap: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '3rem', marginTop: 'auto', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '2rem' }}>
                    <div>
                        <span style={{ fontSize: '9px', opacity: 0.3, marginBottom: '5px', display: 'block' }}>PORTAL STATUS</span>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                             <Network size={14} color="var(--primary)" />
                             <span style={{ fontSize: '13px', fontWeight: 'bold' }}>ONLINE</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style={{ flex: 1, background: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                ...
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 60, repeat: Infinity, ease: 'linear' }} style={{ position: 'absolute', width: '600px', height: '600px', border: '1px solid rgba(74, 222, 128, 0.04)', borderRadius: '50%' }} />
                <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}>
                    <Triangle size={360} fill="rgba(74, 222, 128, 0.03)" color="rgba(74, 222, 128, 0.15)" strokeWidth={0.5} />
                </motion.div>
            </div>
        </div>
    );
